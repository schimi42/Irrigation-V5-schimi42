...
class Zone(SwitchEntity, RestoreEntity):
    ...
    async def async_added_to_hass(self):
        ...
        if self._zonedata.ignore_sensors is not None:
            self._extra_attrs[ATTR_IGNORE_SENSOR] = self._zonedata.ignore_sensors.entity_id
    ...
    async def prepare_to_run(self,scheduled=True):
        ...
        self._remaining_time = await self.calc_run_time(repeats=self.repeat,scheduled=scheduled)
    ...
    async def should_run(self, scheduled=True):
        ...
        if self.status.state in [CONST_DISABLED, CONST_PROGRAM_DISABLED,
                     CONST_UNAVAILABLE, CONST_ADJUSTED_OFF,
                     CONST_NO_WATER_SOURCE, CONST_RAINING, CONST_ZONE_DISABLED]:
            return False
        if not scheduled:
            return True
    ...
    async def handle_validation_error(self):
        ...
        if v_error in (CONST_OFF, CONST_ON, CONST_PENDING, CONST_ECO):
            return v_error
        if self._status in (CONST_ON, CONST_PENDING, CONST_ECO) and v_error == CONST_RAINING:
            return self._status
    ...
    async def next_run_validation(self):
        ...
        if self.rain_sensor == CONST_ON and not self.ignore_sensors and self.rain_behaviour == 'continue':
    ...
    async def calc_next_run(self):
        ...
        if self.last_ran.native_value is None:
            ...
            v_last_ran  = self.last_ran.native_value.replace(hour=starthour, minute=startmin, second=00, microsecond=00)
        ...
        try: # Frq is numeric
            ...
            today_start_time = dt_util.as_local(dt_util.now()).replace(hour=starthour, minute=startmin, second=00, microsecond=00)
            today_begin = dt_util.as_local(dt_util.now()).replace(hour=00, minute=00, second=00, microsecond=00)
            ...
            if (today_start_time - v_last_ran).total_seconds()/86400 >= frq:
                ...
                v_next_run = dt_util.as_local(dt_util.now()).replace(hour=starthour, minute=startmin, second=00, microsecond=00)
                if today_start_time < dt_util.as_local(dt_util.now()):
                    v_next_run += timedelta(days=1)
            elif today_start_time >= dt_util.as_local(dt_util.now()) and last_ran_day_begin == today_begin:
            ...
            v_last_ran = dt_util.as_local(dt_util.now()).replace(hour=starthour, minute=startmin, second=00, microsecond=00)
            today = v_last_ran.isoweekday()
            v_next_run = v_last_ran + timedelta(days=100) #arbitary max
            for day in string_freq :
                # try:
                    if self.get_weekday(day) == today and v_last_ran > dt_util.as_local(dt_util.now()):
    ...
    async def calc_run_time(self, seconds_run=0, volume_delivered=0, repeats=1, scheduled=True):
        ...
        for reps in range(self.repeat, 0, -1):
            ...
            if self.wait > 0 and reps > 1:
                ...
                for _ in range(self.wait):
                    seconds_run += 1
                    self._remaining_time = await self.calc_run_time(seconds_run, repeats=reps, scheduled=self.scheduled)
                    await self.remaining_time.set_value(self._remaining_time)
                    if self._stop:
                        self._aborted = True
                        break
    ...
    async def volume(self,water_adjust_value,reps):
        ...
        while volume_remaining > 0:
            ...
            self._remaining_time = await self.calc_run_time(volume_delivered=volume_delivered,
                                                            repeats=reps,
